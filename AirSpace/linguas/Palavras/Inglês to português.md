## **1. ADVÉRBIOS INTERROGATIVOS**

- **What** = o que?
    
- **Where** = onde?
    
- **When** = quando?
    
- **Why** = por que?
    
- **Which** = qual?
    
- **Who** = quem?
    
- **How** = como?
    

---

## **2. VERBOS**

**Formato: Presente = Tradução | Passado = Tradução | Futuro (will + verbo) = Tradução**

- **Listen** = escutar  
    Passado: Listened = escutei  
    Futuro: Will listen = escutará
    
- **Spell** = soletrar  
    Passado: Spelled = soletrei  
    Futuro: Will spell = soletrará
    
- **Write** = escrever  
    Passado: Wrote = escrevi  
    Futuro: Will write = escreverá
    
- **Repeat** = repetir  
    Passado: Repeated = repeti  
    Futuro: Will repeat = repetirá
    
- **Put** = colocar  
    Passado: Put = coloquei  
    Futuro: Will put = colocará
    
- **Know** = saber, conhecer  
    Passado: Knew = soube, conheci  
    Futuro: Will know = saberá, conhecerá
    
- **Have** = possuir, ter  
    Passado: Had = tive  
    Futuro: Will have = terá
    
- **Get** = pegar, obter, receber, conseguir, adquirir, ganhar  
    Passado: Got = peguei, obtive, recebi  
    Futuro: Will get = pegará, receberá
    
- **Look** = olhar, ver, observar  
    Passado: Looked = olhei  
    Futuro: Will look = olhará
    
- **Decide** = decidir  
    Passado: Decided = decidi  
    Futuro: Will decide = decidirá
    
- **Resolve** = resolver  
    Passado: Resolved = resolvi  
    Futuro: Will resolve = resolverá
    
- **Are** = são  
    Passado: Were = eram  
    Futuro: Will be = serão
    
- **Be** = ser, estar, haver, ficar  
    Passado: Was/Were = fui, estava  
    Futuro: Will be = será, estará
    
- **Choose** = escolher  
    Passado: Chose = escolhi  
    Futuro: Will choose = escolherá
    
- **Has** = tem  
    Passado: Had = tinha  
    Futuro: Will have = terá
    
- **Learn** = aprender, estudar  
    Passado: Learned = aprendi  
    Futuro: Will learn = aprenderá
    
- **Make** = fazer  
    Passado: Made = fiz  
    Futuro: Will make = fará
    
- **Order** = ordenar, arranjar, mandar  
    Passado: Ordered = ordenei  
    Futuro: Will order = ordenará
    
- **Spend** = passar, pagar, gastar  
    Passado: Spent = passei, paguei, gastei  
    Futuro: Will spend = passará, gastará
    
- **Fill** = preencher, ocupar  
    Passado: Filled = preenchi  
    Futuro: Will fill = preencherá
    
- **See** = ver  
    Passado: Saw = vi  
    Futuro: Will see = verá
    
- **Come** = vir  
    Passado: Came = vim  
    Futuro: Will come = virá
    
- **Say** = dizer, declarar  
    Passado: Said = disse  
    Futuro: Will say = dirá
    
- **Tell** = dizer, contar, informar  
    Passado: Told = disse, contei  
    Futuro: Will tell = dirá, contará
    
- **Understand** = entender  
    Passado: Understood = entendi  
    Futuro: Will understand = entenderá
    
- **Speak** = falar  
    Passado: Spoke = falei  
    Futuro: Will speak = falará
    
- **Warm** = esquentar  
    Passado: Warmed = esquentei  
    Futuro: Will warm = aquecerá
    
- **Form** = formar, formular  
    Passado: Formed = formei  
    Futuro: Will form = formará
    
- **Remember** = lembrar  
    Passado: Remembered = lembrei  
    Futuro: Will remember = lembrará
    
- **Communicate** = comunicar  
    Passado: Communicated = comuniquei  
    Futuro: Will communicate = comunicará
    
- **Collect** = coletar  
    Passado: Collected = coletei  
    Futuro: Will collect = coletará
    
- **View** = ver, visualizar  
    Passado: Viewed = visualizei  
    Futuro: Will view = visualizará
    
- **Create** = criar  
    Passado: Created = criei  
    Futuro: Will create = criará
    
- **Stay** = ficar  
    Passado: Stayed = fiquei  
    Futuro: Will stay = ficará
    
- **Go** = ir  
    Passado: Went = fui  
    Futuro: Will go = irá 
    Terceira pessoa = goes = ele vai 
    
- **Eat** = comer  
    Passado: Ate = comi  
    Futuro: Will eat = comerá
    
- **Thought**  = pensei
	**Presente:** Think = pensar  
	**Passado:** Thought = pensei  
	**Futuro:** Will think = pensará

- Forget = esquecer
	Presente = forget = esquecer
	Passado = forgot = esqueci
	Futuro = will forget = esquecera 
	
- Give = dar
	Presente = give = dar
	Passado = gave = dei, deu
	Futuro = will give = dará
	
- Pay = pagar
    Presente = pay = pagar
    Passado = paid = paguei
    Futuro = will pay = pagarei
    3° pessoa = pays

- Arrive = chegar
	Presente = arrive = chegar
	Passado = arrived = cheguei
	Futuro = will arrive = chegarei

- Feel = sentir
	**Presente**: feel = sentir, sentir-se 
    **Passado**: felt = senti
    **Futuro**: will feel = sentirá

- Being = sendo, estar sendo
   **Presente**: being = sendo / estando  
   **Passado**: been = sido / estado  
   **Futuro**: will be being = estará sendo

- Going = indo
    **Presente**: going = indo  
    **Passado**: went = foi / fui  
    **Futuro**: will be going = estará indo

- Try = tentar, experimenta
    **Presente**: try = tentar, experimentar
    **Passado**: tried = tentei, experimentei
    **Futuro**: will try = tentará, experimentará

- Attended = assistir, comparecer, participar, frequentar
    **Presente**: attend = assistir, comparecer, participar, frequentar
    **Passado**: **attended** = assisti, compareci, participei, frequentei
    **Futuro**: will attend = assistirá, comparecerá, participará 

- Wear = usar, vestir
    **Presente**: wear = usar, vestir
    **Passado**: **wore** = usei, vesti
    **Futuro**: will wear = usará, vestirá

- Walk = andar, caminhar
    **Presente**: walk = andar, caminhar
    **Passado**: walked = andei, caminhei    
    **Futuro**: will walk = andará, caminhará

- Like = gostar
    **Presente**: like = gostar
    **Passado**: liked = gostei
    **Futuro**: will like = gostará

- Practice = praticar
    **Presente**: practice = praticar, treinar
    **Passado**: practiced = pratiquei
    **Futuro**: will practice = praticará

- Buy = comprar
    **Presente**: buy = comprar
    **Passado**: bought = comprei
    **Futuro**: will buy = comprará

- Take = pegar
    **Presente**: take = pegar, tomar, levar
    **Passado**: took = peguei, tomei, levei
    **Futuro**: will take = pegará, tomará, levará


---

## 2.1 **VERBO MODAL 

- Should
	Função: indicar conselho, recomendação, obrigação leve, expectativa ou probabilidade

| Uso                           | Exemplo                       | Tradução                             |
| ----------------------------- | ----------------------------- | ------------------------------------ |
| **Conselho/recomendação**     | You **should** see a doctor.  | Você **deveria** ver um médico.      |
| **Obrigação leve**            | We **should** respect others. | Nós **devemos** respeitar os outros. |
| **Expectativa/probabilidade** | It **should** rain today.     | **Deve** chover hoje.                |
| **Correção**                  | You **shouldn't** say that.   | Você **não deveria** dizer isso.     |

---
- Shall 
  - **Função:** Indicar futuro, sugestão, oferta, obrigação ou determinação
  - **Tradução:** vou, vamos, deverei, deveremos (depende do contexto)



### 📝 **Conjugação:**
## **3. SUBSTANTIVOS**

- **Letter** = letra, carta
    
- **Numbers** = números
    
- **Order** = ordem
    
- **Many** = muitos, multidão
    
- **End** = fim
    
- **Endings** = finais
    
- **Key** = chave
    
- **Car** = carro
    
- **Wallet** = carteira
    
- **Memory stick** = pen drive
    
- **Purse** = bolsa
    
- **Noise** = barulho
    
- **Rummaging** = vasculhando
    
- **Look** = olhar
    
- **Picture** = foto
    
- **Recording** = gravação
    
- **Statements** = declarações
    
- **Hair** = cabelo
    
- **Sentence** = frase
    
- **Blonde** = loira, loiro
    
- **Match** = combinar, correspondência
    
- **Paragraph** = parágrafo
    
- **Piece** = pedaço
    
- **Phone** = telefone
    
- **Spelling** = ortografia, soletração
    
- **Meeting** = reunião
    
- **T-shirt** = camiseta
    
- **Uncle** = tio
    
- **Aunt** = tia
    
- **Cousin** = primo
    
- **Daughter** = filha
    
- **Children** = criança
    
- **Husband** = marido
    
- **Partner** = parceiro, parceira
    
- **Saying** = ditado, pronúncia
    
- **Waiter** = garçom, garçonete
    
- **Cook** = cozinheiro
    
- **Watches** = relógios
    
- **Half** = metade
    
- **Quarter** = quarto
    
- **Mistake** = erro
    
- **Spot** = ver, local
    
- **Cheap** = barato
    
- **Accountant** = contador
    
- **Near** = próximo
    
- **Travel** = viagem
    
- **Together** = junto
    
- **Takes** = leva, pega
    
- **Greeting** = introduções
    
- **Warm** = calor, aquecimento
    
- **Answers** = respostas
    
- **Ways** = caminhos, formas, métodos
    
- **Kettle** = chaleira
    
- **Chair** = cadeira
    
- **Steaks** = bifes
    
- **Meat** = carne
    
- **Chicken** = galinha, frango
    
- **Lettuce** = alface
    
- **Fish** = peixe
    
- **Fries** = batata frita
    
- **Pool** = piscina
    
- **Swimming** = natação 

- Meal = refeição

- Recognition = reconhecimento, identificação 

- Being = ser, existência

- Wedding = casamento (cerimônia, evento)
---

## **4. PREPOSIÇÕES**

- **In** = em, de, a, para, dentro
    
- **As** = na qualidade de
    
- **To** = para, a, de, até, em
    
- **With** = com, de, a, em
    
- **In the** = no, na
    
- **According** = de acordo com, conforme
    
- **End in** = terminar em
    
- **What’s in the** = o que tem no/na
    
- **Into** = em, dentro, para dentro de
    
- **At** = no, na, em, a
    
- **At the** = no, nos, nas
    
- **To the** = para o, à
    
- **About** = sobre
    
- **I mean** = quero dizer
    
- **On** = sobre, na, no, em
    
- **I’d like** = eu gostaria
    
- **Is With** = está com
    
- **Is There** = existem, tem
    
- **Between** = entre
    
- **Far** = distante
    
- **Next** = próximo
    
- **Next to** = ao lado de
    
- **Behind** = atrás


---

## **5. CONJUNÇÕES**

- **As** = como, quanto, enquanto, visto que
    
- **If** = se
    
- **But** = mas, porém
    
- **Than** = de, do, do que
    

---

## **6. ADVÉRBIOS**

### **Lugar**

- **There** = ali, lá
- Ahead = à frente, adiante, na dianteira
### **Modo**

- **Then** = então
    
- **Just** = exatamente, apenas
    

### **Intensidade**

- **As** = tão
    
- **Many** = muitas
    
- **Just** = quase, por pouco
    
- **Often** = muitas vezes
    

### **Ordinal**

- **According** = segundo
    

### **Inclusão**

- **Also** = também
    

### **Exclusão**

- **Just** = somente
    

### **Tempo**

- **Just** = há pouco, agora mesmo
    
- **Yet** = ainda
    
- **Until** = até
    
- **Once** = uma vez
    
- Always = sempre

- Sometimes = as vezes 

---

## **7. PRONOMES**

- **Them** = eles, os, as, lhes
    
- **Their** = suas, seus, deles, delas
    
- **Her** = seu, sua, a ela
    
- **Him** = lhe, o, a ele
    
- **Each** = cada, cada um
    
- **It** = isto, ele, ela
    
- **It’s** = isso é
    
- **This** = esse, essa, esta, este, isto
    
- **Of this** = disso, deste
    
- **Themselves** = eles mesmos, a si mesmos
    
- **Someone** = alguém
    
- **Can** = pode, poder
    
- **Some** = algumas, alguns
    
- **Both** = ambas, ambos
    
- **Which** = qual, o qual
    
- **These** = esses, essas, estas, estes
    
- **His** = seu, sua, dele
    
- **Everything** = tudo

- **That** = que, isso, aquele, aquela

- Those = aqueles, aquelas
    

---

## **8. ARTIGOS**

- **The** = os, as, a, o
    
- **An** = um, uma
    

---

## **9. ADJETIVOS**

- **Many** = numerosos
    
- **Careful** = cuidadoso, atento
    
- **Tall** = alto
    
- **Short** = curto, baixo
    
- **Pretty** = bonito
    
- **Thin** = fino, magro
    
- **Curly** = encaracolado, crespo
    
- **Good-looking** = bonito, atraente
    
- **Daily** = diário
    
- **Every** = todo, cada
    
- **Easier** = mais fácil
    
- **Separated** = separado
    
- **Able** = capaz
    
- **Past** = passado

- Bright - brilhante, claro, vivo, inteligente, alegre

---

## **10. LOCUÇÃO INTERROGATIVA DE QUANTIDADE**

- **How many** = quantos?
    

---

## 11. **EXPRESSÕES COMUNS

- Try on = experimentar (roupa)
- Try out = testar, experimentar
- Try your best = dar o seu melhor
- turn the sound up = aumentar o volume
# **TÓPICO EXTRA: SINÔNIMOS E TEMPOS VERBAIS**

## **1. SINÔNIMOS DE VERBOS COMUNS**

- **Listen**: Hear (ouvir), Attend to (prestar atenção)
    
- **See**: Look (olhar), Watch (assistir), Observe (observar)
    
- **Say**: Tell (dizer), State (afirmar), Declare (declarar)
    
- **Understand**: Comprehend (compreender), Grasp (captar)
    
- **Make**: Create (criar), Produce (produzir), Build (construir)
    
- **Get**: Obtain (obter), Receive (receber), Acquire (adquirir)
    
- **Have**: Possess (possuir), Own (ser dono)
    
- **Go**: Move (mover), Travel (viajar), Proceed (prosseguir)
    

## **2. EXEMPLO DE TEMPOS VERBAIS (“MAKE”)**

- **Presente**: I make = eu faço
    
- **Passado**: I made = eu fiz
    
- **Futuro**: I will make = eu farei
    
- **Presente Contínuo**: I am making = estou fazendo
    
- **Presente Perfeito**: I have made = tenho feito
    

## **3. SINÔNIMOS DE ADVÉRBIOS**

- **Then**: Afterward (depois), Subsequently (subsequentemente)
    
- **Just**: Exactly (exatamente), Merely (meramente)
    
- **Often**: Frequently (frequentemente), Repeatedly (repetidamente)
    
- **Also**: Too (também), As well (igualmente)
    

## **4. SINÔNIMOS DE PREPOSIÇÕES**

- **In**: Inside (dentro), Within (dentro de)
    
- **With**: Alongside (ao lado de), Together with (junto com)
    
- **About**: Regarding (a respeito), Concerning (concernente)
    
- **Between**: Among (entre vários), Amidst (no meio de)
    

## **5. SINÔNIMOS DE ADJETIVOS**

- **Tall**: High (alto), Lofty (elevado)
    
- **Short**: Brief (breve), Concise (conciso)
    
- **Pretty**: Beautiful (bonito), Attractive (atraente)
    
- **Careful**: Cautious (cauteloso), Attentive (atento)